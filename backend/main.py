...
def save_receipt(data: ReceiptCreate, db: Session = Depends(get_db), _: None = Depends(require_auth)):
    ...
    return {
        ...
        "amount_gross": float(receipt.amount_gross) if receipt.amount_gross is not None else None,
        "amount_net": float(receipt.amount_net) if receipt.amount_net is not None else None,
        ...
        "foreign_amount": float(receipt.foreign_amount) if receipt.foreign_amount is not None else None,
        "exchange_rate": float(receipt.exchange_rate) if receipt.exchange_rate is not None else None,
    ...
    return [
        {
            ...
            "amount_gross": float(r.amount_gross) if r.amount_gross is not None else None,
            "amount_net": float(r.amount_net) if r.amount_net is not None else None,
